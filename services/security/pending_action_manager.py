...
class PendingActionManager:
    ...
    def create_pending_action(
        ...
        with self._lock:
            ...
            logger.info(
                "Created pending action: id={}, tool={}, risk={}, ttl={}s, session_id={}",
                action_id,
                tool_name,
    ...
    def claim_for_execution(
        ...
        If the pending action was bound to a session_id, cross-session claims fail closed.
        Unbound actions (session_id=None) preserve backward-compatible non-voice execution.
        """
        if not action_id or not isinstance(action_id, str):
        ...
        with self._lock:
            ...
            if action is None:
                return (
                    ...
                    "No pending action found with the specified identifier.",
                )

            # Session binding validation: voice-bound actions fail closed on cross-session claims
            if action.session_id is not None and (
                session_id is None or session_id != action.session_id
            ):
                logger.warning(
                    "Cross-session claim rejected for action {}: action_session={}, claim_session={}",
                    action_id,
                    action.session_id,
                    session_id,
                )
                return (
                    ...
                    f"Session mismatch: action is bound to session '{action.session_id}' and cannot be claimed across sessions.",
                )
            ...
            if action.state == ActionState.PENDING and action.is_expired():
                ...
                return (
                    ...
                    "The pending action has expired and cannot be executed.",
                )
            ...
            return (
                ...
                "Action successfully claimed for execution.",
            )
    ...
    def cancel_action(
        ...
        with self._lock:
            ...
            if (
                ...
                and session_id != action.session_id
            ):
                return ConfirmationResult(
                    success=False,
                    status=ConfirmationStatus.INVALID,
                    action_id=action_id,
                    message="Session mismatch: action is bound to another session and cannot be cancelled across sessions.",
